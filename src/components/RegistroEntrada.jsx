...
const RegistroEntrada = () => {
        ...
        try {
            ...
            if (!respuesta.ok) {
                if (respuesta.status === 409) {
                    setMensaje({
                        texto: "El visitante ya se encuentra dentro de la empresa",
                        tipo: "error",
                    });
                } else {
    ...
    const validarFormulario = async () => {
        if (!documento || !nombre || !telefono || !dependencia || !funcionario) {
            setMensaje({ texto: "Por favor completa todos los campos", tipo: "error" });
            return;
        }

        try {
            const response = await fetch(`${API_URL}/api/visitantes/entrada/validar/${documento}`);
            ...
            if (!response.ok) {
                throw new Error("Error al validar entrada");
            }

            setMostrarModal(true);
        } catch (error) {
            console.error("Error:", error);
            setMensaje({ texto: "Error al validar la entradaaa", tipo: "error" });
        }
    };
